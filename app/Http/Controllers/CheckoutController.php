...
class CheckoutController extends Controller
{
    ...
    public function customerSignUp(Request $request){
        ...
        if($request->cus_res_one){
            return redirect()->route('checkout-shipping'); 
        }else if($request->cus_res_two){
    ...
    public function customerLoginCheck(Request $request){
        ...
        Session::put('customerName',$customer->first_name.' '.$customer->last_name);
        if($request->cus_login_one){
            return redirect()->route('checkout-shipping'); 
        }else if($request->cus_login_two){
            return redirect()->route('/');
        }
        //return redirect('/checkout/shipping');
        } else {
            Session::flash('invalid_email_or_password','Please, Enter valid Email Address or Password.');
            if($request->cus_login_one){
                return redirect()->route('checkout');
            }else if($request->cus_login_two){
                return redirect()->route('new-customer-login');
            }
    ...
    public function newOrder(Request $request){
        ...
        if ($paymentType=='cash') {
            ...
            Cart::destroy();
            return redirect()->route('ssl-commerz-order');
        }
        else if($paymentType=='paypal'){
            $this->add_info_to_database();
            return redirect()->route('complete-order');
            //return redirect('/complete/order');
        }
